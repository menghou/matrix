...
// Factorize computes the higher order generalized singular value decomposition (GSVD)
// of the n input r_i×c column matrices in m.
//
//  M_0 = U_0 * Σ_0 * V^T
//  M_1 = U_1 * Σ_1 * V^T
//  .
...
func (gsvd *HOGSVD) Factorize(m ...Matrix) (ok bool) {
	...
	for i, ai := range a {
		for _, aj := range a[i+1:] {
			gsvd.err = sij.Solve(ai.T(), aj.T())
			if gsvd.err != nil {
				return false
			}
			s.Add(s, sij.T())

			gsvd.err = sij.Solve(aj.T(), ai.T())
			if gsvd.err != nil {
				return false
			}
			s.Add(s, sij.T())
